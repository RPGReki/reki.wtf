{:.icons.shields.justify-content-start.no-print}
[![Discord]({{ "/proxy/shields/-/discord/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://discord.gg/8DCZWGc){:.no-print rel="me"}
[![Mastodon]({{ "/proxy/shields/-/mastodon/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://gamingjp.org/@0xReki){:rel="me"}
[![twitter]({{ "/proxy/shields/-/twitter/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://twitter.com/0xReki){:rel="me"}
[![Patreon]({{ "/proxy/shields/-/patreon/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://www.patreon.com/bePatron?u=27792463){:.no-print rel="me"}
[![ScribbleHub]({{ "/proxy/shields/-/scribblehub/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://www.scribblehub.com/profile/19792/0xreki/){:.no-print rel="me"}
[![Tapas]({{ "/proxy/shields/-/tapas/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://tapas.io/0xReki){:.no-print rel="me"}
[![Twitch]({{ "/proxy/shields/-/twitch/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://twitch.tv/RPGReki){:.no-print rel="me"}
[![GitHub]({{ "/proxy/shields/-/github/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://github.com/0xReki){:rel="me"}
[![GitLab]({{ "/proxy/shields/-/gitlab/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://gitlab.com/0xReki){:rel="me"}
[![Flattr]({{ "/proxy/shields/-/flattr/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://flattr.com/@0xReki){:.no-print rel="me"}
[![LinkedIn]({{ "/proxy/shields/-/linkedin/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://linkedin.com/in/q-thai-chung){:rel="me"}
[![XING]({{ "/proxy/shields/-/xing/shield.svg" | prepend: site.static_url | absolute_url }}){:loading="lazy" width="64" height="64"}](https://www.xing.com/profile/Thai_Chung){:rel="me"}
